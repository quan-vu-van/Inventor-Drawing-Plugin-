        ...
        private void CreateRibbonButton(bool firstTime)
        {
            try
            {
                ...
                try {
                    ...
                    using (Stream s16 = asm.GetManifestResourceStream("InventorDrawingPlugin.Resources.icondata16.png"))
                    using (Stream s32 = asm.GetManifestResourceStream("InventorDrawingPlugin.Resources.icondata32.png")) {
                ...
                try {
                    _toggleButtonDef = (ButtonDefinition)controlDefs["InventorDrawingPlugin.ToggleButton"];
                } catch {
                    _toggleButtonDef = controlDefs.AddButtonDefinition(
                        "Drawing\nTools", "InventorDrawingPlugin.ToggleButton", 
                        CommandTypesEnum.kShapeEditCmdType, 
                        "{9D0B4403-518D-40C6-98AC-CD2FF878B309}", // GUID MỚI
                        "Công cụ xử lý bản vẽ", "Drawing Tools", icondata16, icondata32);
                }

                _toggleButtonDef.OnExecute += (NameValueMap Context) => { if (_dockableWindow != null) _dockableWindow.Visible = !_dockableWindow.Visible; };
                ...
                foreach (string envName in targetEnvironments) {
                    try {
                        ...
                        bool exists = false;
                        foreach (CommandControl ctrl in panel.CommandControls) { if (ctrl.InternalName == "InventorDrawingPlugin.ToggleButton") exists = true; }
